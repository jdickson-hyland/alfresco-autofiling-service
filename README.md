...
|--------|-----|-------------|
| `GET` | `/alfresco/service/alfresco/autofiling/rules` | List all rules |
| `POST` | `/alfresco/service/alfresco/autofiling/rules` | Create a rule |
| `GET` | `/alfresco/service/alfresco/autofiling/rules/{storeType}/{storeId}/{nodeId}` | Get a rule by NodeRef |
| `PUT` | `/alfresco/service/alfresco/autofiling/rules/{storeType}/{storeId}/{nodeId}` | Update a rule |
| `DELETE` | `/alfresco/service/alfresco/autofiling/rules/{storeType}/{storeId}/{nodeId}` | Delete a rule |
| `POST` | `/alfresco/service/alfresco/autofiling/rules/{storeType}/{storeId}/{nodeId}/run` | Trigger a rule immediately |
...
curl -u admin:admin -X POST \
  ...
  -d @rule.json \
  http://localhost:8080/alfresco/service/alfresco/autofiling/rules
```
...
```bash
curl -u admin:admin -X POST \
  http://localhost:8080/alfresco/service/alfresco/autofiling/rules/workspace/SpacesStore/<nodeId>/run
```
...
| Logger | Level | Events |
| ------ | ----- | ------ |
| `org.alfresco.autofiling.job.AutofilingJob` | INFO | Lock acquired, execution complete |
| `org.alfresco.autofiling.job.AutofilingJobWorker` | INFO | Job start / complete |
| `org.alfresco.autofiling.bootstrap.AutofilingBootstrap` | INFO | Bootstrap start / complete |
| `org.alfresco.autofiling.service.AutofilingServiceImpl` | INFO | Enabled rule count, per-rule filed count |
| `org.alfresco.autofiling.service.AutofilingServiceImpl` | DEBUG | Per-rule entry, batch limit reached, document skipped (age/already moved) |
| `org.alfresco.autofiling.service.AutofilingRuleServiceImpl` | INFO | Rule created / updated / deleted |
| `org.alfresco.autofiling.service.AutofilingRuleServiceImpl` | DEBUG | Node refs, list counts |
| `org.alfresco.autofiling.service.PathResolver` | DEBUG | Resolved template → path per document |
| `org.alfresco.autofiling.service.FolderPathServiceImpl` | DEBUG | Folders created during path resolution |
| `org.alfresco.autofiling.webscript.*` | INFO | Mutating API calls (create, update, delete, run) |
| `org.alfresco.autofiling.webscript.*` | DEBUG | Read API calls (get, list) |
| `org.alfresco.autofiling.webscript.AbstractAutofilingWebScript` | WARN | All 4xx error responses (includes HTTP status and message) |

To enable DEBUG logging for the entire module, add to `alfresco-global.properties`:

```properties
log4j2.logger.autofiling.name=org.alfresco.autofiling
...
```bash
mvn test -pl autofiling-service-platform
```
...
# Alfresco AIO Project - SDK 4.11

This is an All-In-One (AIO) project for Alfresco SDK 4.11.
